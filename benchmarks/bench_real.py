...
# ── Suppression warning KMeans / MKL memory leak sur Windows ─────────────────
...
# Dossier de bench_real.py lui-même → learning_curves_one_dataset.py
_bench_dir = this_file.parent
...
import alframework.strategies.active_pseudolabelv55
import alframework.strategies.active_pseudolabelv56
import alframework.strategies.active_pseudolabelv57
...
def _compute_from_train_test(
    *,
    ...
    for i_strategy, name in enumerate(strategy_names, start=1):
        ...
        _ov = dict(overrides)
        try:
            if "random_state" in _inspect.signature(STRATEGIES[name].__init__).parameters:
                _ov["random_state"] = int(init_random_state)
        except (ValueError, TypeError):
            pass
        strat = STRATEGIES[name](**_ov)
            ...
            if history_dir is not None:
                ...
                for strategy_name, strat_out in out["strategies"].items():
                    ...
                    history_path = history_dir / (
                        f"history_{model_kind}_{safe_dataset}_split{split_global_id}_{safe_strategy}.json"
                    )
...
def main():
    ...
    # -------------------------------------------------------------------------
    STRATEGIES_TO_TEST = [
        # Panel aligné sur bench_synthetic (18 méthodes, toutes écoles)
        # -- incertitude pure --
        "random",
        "margin",
        "entropy",
        ...
        # selftrain_acq exclu (ne remplit pas le budget à seuil fixe)
    ]

    STRATEGY_OVERRIDES = {
        # incertitude pure : pas d'override (defaults)
        # diversité pure
        "typiclust": {"neighbors": 5, "random_state": 0},
        "probcover": {"delta_quantile": 0.10, "knn_k": 10, "random_state": 0},
        # hybrides
        "badge_approx": {"random_state": 0},
        "unc_feature_kmeans": {"uncertainty_metric": "margin", "n_clusters_factor": 3},
        "diversity_optimized_batch": {"uncertainty_metric": "margin", "lambda_diversity": 0.5},
        # comité
        "qbc": {"n_committee": 5, "bootstrap_ratio": 1.0, "metric": "vote_entropy"},
        "robust_qbc": {"n_committee": 5, "bootstrap_ratio": 0.8, "metric": "confidence_weighted"},
        "tri_committee": {"bootstrap_ratio": 1.0, "mode": "two_vs_one"},
        "adaptive_disagreement": {"n_committee": 5, "min_disagreement": 0.2},
        # famille unifiée
        "dbal": {
            "method": "margin",
            "dbal_factor": 5,
        },
        "rank2022": {"s_factor": 10, "linkage": "ward", "loss": "margin"},
        "ActivePseudoLabelV58": {
    ...
    "debug_sat": False,
      ...
      "ActivePseudoLabelV57": {  # ou ("ActivePseudoLabelV57", {...}) pour PANEL
    "lambda_prop": 0.0,
    "source_policy": "auto_fast",
    "max_u_contrast_for_source": 0.28,
    "min_u_flatness_for_source": 0.88,
    ...
    model_kind = "rf"   # "rf" (RandomForest) ou "lr" (LogisticRegression)
    ...
    bench_root = project_root / "tests" / "benchmark_data" / "reeltabulaire"
