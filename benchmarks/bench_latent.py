...
# ─── Réglage threads AVANT tout import de numpy/sklearn ─────────────────────────
# Supprime le UserWarning « KMeans is known to have a memory leak on Windows with
# MKL ... ». Doit impérativement être défini AVANT l'import de numpy/sklearn pour
# être pris en compte par MKL/OpenMP. setdefault respecte une valeur déjà fixée
...
def main():
    # Classifieur : "rf" (RandomForest) ou "lr" (LogisticRegression)
    model_kind = "lr"
    # -------------------------------------------------------------------------
    # Affichage
    # -------------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------------
    STRATEGIES_TO_TEST = [
        # Panel aligné sur bench_synthetic / bench_real (18 méthodes)
        # -- incertitude pure --
        "random",
        "margin",
        "entropy",
    ...
    STRATEGY_OVERRIDES = {
        ...
        # famille unifiée
        "dbal": {"method": "margin", "dbal_factor": 5},
